...
public class MainActivityMetro extends AppCompatActivity
{
    TimePicker tp;
    String sourceStn;
    String destStn;
   ...
   int count=0;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_mainmetro);
        ...
        if (sourceStn.equalsIgnoreCase("Versova")) {
            FirebaseRecyclerOptions<model> options =
                    new FirebaseRecyclerOptions.Builder<model>()
                            .setQuery(FirebaseDatabase.getInstance().getReference().child("Metro").child("MetroDown").orderByChild("Source").equalTo("Versova"), model.class)
                            .build();
            adapter = new myadapter(options);
            recview.setAdapter(adapter);
            count=1;
        } else if (sourceStn.equalsIgnoreCase("Ghatkopar")) {
            FirebaseRecyclerOptions<model> options =
                    new FirebaseRecyclerOptions.Builder<model>()
                            .setQuery(FirebaseDatabase.getInstance().getReference().child("Metro").child("MetroUp").orderByChild("Source").equalTo("Ghatkopar"), model.class)
                            .build();
            adapter = new myadapter(options);
            recview.setAdapter(adapter);
            count=2;
   ...
   public void speakDetails() {
       ...
       df = FirebaseDatabase.getInstance().getReference().child("Metro").child("MetroUp");
