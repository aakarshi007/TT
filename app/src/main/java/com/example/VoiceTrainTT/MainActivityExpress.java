...
public class MainActivityExpress extends AppCompatActivity
{
   ...
   int count=0;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (destStn.equalsIgnoreCase("New Delhi")) {
            FirebaseRecyclerOptions<model> options =
                    new FirebaseRecyclerOptions.Builder<model>()
                            .setQuery(FirebaseDatabase.getInstance().getReference().child("Express").child("ExpressUp").orderByChild("Destination").equalTo("New Delhi"), model.class)
                            .build();
            adapter = new myadapter(options);
            recview.setAdapter(adapter);
            count=1;
        } else  if (destStn.equalsIgnoreCase("Varanasi")) {
            FirebaseRecyclerOptions<model> options =
                    new FirebaseRecyclerOptions.Builder<model>()
                            .setQuery(FirebaseDatabase.getInstance().getReference().child("Express").child("ExpressUp").orderByChild("Destination").equalTo("Varanasi"), model.class)
                            .build();
            adapter = new myadapter(options);
            recview.setAdapter(adapter);
            count=1;
